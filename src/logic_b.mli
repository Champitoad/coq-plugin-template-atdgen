...
val write_untagged_proof :
  ...
  (** Output an untagged biniou value of type {!type:proof}. *)

val write_proof :
  Bi_outbuf.t -> proof -> unit
  (** Output a biniou value of type {!type:proof}. *)

val string_of_proof :
  ?len:int -> proof -> string
  (** Serialize a value of type {!type:proof} into
      a biniou string. *)
...
val get_proof_reader :
  ...
  (** Return a function that reads an untagged
      biniou value of type {!type:proof}. *)

val read_proof :
  Bi_inbuf.t -> proof
  (** Input a tagged biniou value of type {!type:proof}. *)

val proof_of_string :
  ?pos:int -> string -> proof
  (** Deserialize a biniou value of type {!type:proof}.
      @param pos specifies the position where
                 reading starts. Default: 0. *)
